...
$('.task-add-btn').click(function(){
	...
	if (selected_tasks.indexOf(selected_task.val()) == -1) {
		...
		$('#task_list').append(
			...
			'<td><input type="hidden" name="selected_tasks[][\'start_time\']" value="' + selected_start_time.val() + '" />' + selected_start_time.text() +' h</td><td>' + getTaskDuration(selected_task.val()) + '</td>'+
